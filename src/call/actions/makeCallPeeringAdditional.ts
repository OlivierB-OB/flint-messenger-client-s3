...
export const makeCallPeeringAdditional = action(
  (conversationId: string, target: string, purpose: IPeeringPurpose, requiredPeering: string[]) => {
    return async (dispatch: ThunkDispatch<IAppState, void, Action>, getState: () => IAppState) => {
      console.log(`========== makeCallPeeringAdditional: ${target} - ${purpose}`);
      assertValidConversationId(getState(), conversationId);

      requiredPeering.forEach((required) =>
        dispatch(makeCallPeeringInitiate(conversationId, required, purpose, false)),
      );
    };
  },
);
