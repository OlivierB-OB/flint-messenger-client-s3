...
export const makeCallPeeringFinalized = action((
  ...
  return async (dispatch: ThunkDispatch<IAppState, void, Action>, getState: () => IAppState) => {
    ...
    const requiredPeering = getState().call.remotes.filter(({ pendingJoin }) => !pendingJoin)
      .map(({ target }) => target);
    
    dispatch(updateCallRemote({ ...remote, pendingJoin: false }));

    if (requiredPeering.length) dispatch(makeEmit('call-additional-peering-required', { conversationId, target, purpose, requiredPeering }));
  };
});
